/* ************************************************************************
> File Name:     多进程网络服务框架.cpp
> Author:        wang
> Created Time:  Sun Dec 12 15:42:51 2021
> Description:   
 ************************************************************************/
...
bool CTcpServer::Accept(){
    ...
    if((m_connfd = accept(m_listenfd, (struct sockaddr*)&m_clientaddr, (socklen_t*)&m_socklen)) < 0) return false;
    return true;
}
...
*/
    m_buflen = 0;
    return (TcpReadRead(m_connfd, buffer, &m_buflen));
}
...
bool TcpRead(const int sockfd, char* buffer, int* ibuflen, const int itimeout){
    ...
    (*ibuflen) = ntohl(*ibuflen);   //把网络字节序转为主机字节序
...
bool TcpWrite(const int sockfd, const char* buffer, const int ibuflen){
    ...
    memcpy(strTBuffer, &ilenn, 4);          //memcpy内存拷贝 strcpy只能拷贝文本
